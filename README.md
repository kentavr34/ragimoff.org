...
1. **`index.html` (корневой, AZ) — ЭТАЛОН.** НЕ менять без явного разрешения автора.
   ...
   **ВАЖНО:** index.html содержит inline header/mobile-nav/footer (НЕ использует партиалы).
   При правке `_partials/header.html`, `_partials/mobile-nav.html`, `_partials/footer.html`
   или `_i18n.json` — изменения **не применяются автоматически** к index.html.
   Нужно **вручную** синхронизировать тот же блок в index.html, иначе появится дрейф.
   Это сознательное решение автора (см. историю чата 2026-05-07).

...
**KRİTİK:**
<<<<<<< HEAD
1. **Ümumi CSS/JS dublikatları** — bəzi səhifələrdə (xüsusən `index.html`) shared-lə eyni funksiyalar və stillər yenidən yazılıb
...
2. **Mobil menyu standartı fərqlidir** — bir çox səhifədə `.mobile-nav.open`, `index.html`-də isə `.mobile-nav.show` (inline CSS/JS ilə)
...
3. **`shared.js` vs inline script** — `index.html`-də `toggleMenu/closeMenu/IntersectionObserver/submitBooking` kimi adlar təkrar var
...
=======
- [ ] Dəyişikliyi etmədən əvvəl faylı `view` ilə oxu
- [ ] Dəyişiklik etdikdən sonra `grep` ilə yoxla

Hər kodlama sessiyasından SONRA:
- [ ] Hero foto düzgün proporsiyadadır?
- [ ] Azərbaycan hərfləri düzgün görünür?
>>>>>>> 0ffe5554457445a2e1521c38986e7992a05c134b

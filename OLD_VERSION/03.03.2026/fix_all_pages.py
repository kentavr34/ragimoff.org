...
def build_header(active):
  ...
  <div class="header-inner">
    ...
    <nav class="desktop-nav">
      <a href="index.html"{a if active=="ana" else ""}>Ana S\u0259hif\u0259</a>
      <a href="tehsil.html"{a if active=="tehsil" else ""}>T\u0259hsil</a>
      <a href="index.html#services"{a if active=="konsultasiya" else ""}>Konsultasiya</a>
      <a href="b2b.html"{a if active=="korporativ" else ""}>Korporativ</a>
      <a href="blog.html"{a if active=="blog" else ""}>Blog</a>
...
STANDARD_FOOTER = '''\u003cfooter\u003e
  \u003cdiv class="footer-inner" style="max-width:1300px; margin:0 auto; padding:80px 40px;"\u003e
    ...
    \u003cdiv style="border-top:1px solid rgba(255,255,255,0.08); padding-top:40px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:16px;"\u003e
      \u003cp style="font-size:0.82rem; color:rgba(255,255,255,0.35);"\u003e\u00a9 2026 RAGIMOFF Pe\u015f\u0259kar Psixologiya M\u0259kt\u0259bi. Pe\u015f\u0259kar N\u00fcfuzun \u00dcnvan\u0131.\u003c/p\u003e
      \u003ca href="https://www.psychotherapyru.com" target="_blank" style="font-size:0.82rem; color:rgba(255,255,255,0.4); text-decoration:none;"\u003eRusskaya versiya \u2192\u003c/a\u003e
    \u003c/div\u003e
  \u003c/div\u003e
\u003c/footer\u003e
...
MOBILE_NAV_CSS = '''
    ...
    .mobile-nav a {
      color: rgba(255,255,255,0.8); text-decoration: none; font-size: 0.9rem; font-weight: 700;
      padding: 16px 0; border-bottom: 1px solid rgba(255,255,255,0.08); text-transform: uppercase;
      letter-spacing: 1px; transition: color 0.2s;
    }
    .mobile-nav a:hover { color: var(--accent); }
    .mobile-nav .btn-primary { 
      margin-top: 16px; text-align: center; border: none; 
      background: var(--accent); color: var(--navy); padding: 16px; font-weight: 800;
    }
    .mobile-nav-close { 
      background: none; border: none; color: rgba(255,255,255,0.6); cursor: pointer; 
      margin-bottom: 20px; align-self: flex-end; padding: 4px;
    }
    ...
    .wa-float {
      ...
      box-shadow: 0 8px 25px rgba(37,211,102,0.4); z-index: 1500; transition: transform 0.3s;
    }
    ...
    /* === LOGO BOX === */
    .logo-box { 
      height: 60px; padding: 5px; background: rgba(255,255,255,0.05); border-radius: 4px; 
      display: flex; align-items: center; border: 1px solid rgba(181,155,114,0.1); text-decoration: none; 
    }
...
ACTIVE_MAP = {
    ...
    'blog.html': 'blog',
    'b2b.html': 'korporativ',
    'aile-terapiyasi.html': 'konsultasiya',
    'aile-terapiyasi-usaq.html': 'konsultasiya',
    'enurez.html': 'konsultasiya',
    'panik-ataklar.html': 'konsultasiya',
    'depressiya.html': 'konsultasiya',
    'sosial-fobiya.html': 'konsultasiya',
    'proqram.html': 'tehsil',
}
...
def process_file(path):
    ...
    active = ACTIVE_MAP.get(name, 'ana')
    ...
    # 1. Remove Playfair Display import lines
    c = re.sub(
    ...
    # 2. Add mobile nav + footer CSS if missing
    if '.mobile-nav {' not in c:
    ...
    # 3. Replace <header>...</header> + optional mobile-nav div  
    new_hdr = build_header(active)
    # Match: <header>...</header> optionally followed by any mobile-nav divs
    hdr_pat = r'<header>.*?</header>(\s*<div[^>]*(?:mobile-nav|burger)[^>]*>.*?</div>)*'
    ...
    # 4. Replace footer + wa-float
    # Match <footer>...</footer> optionally followed by a wa-float link
    ft_pat = r'<footer>.*?</footer>(\s*<a[^>]*class="wa-float"[^>]*>.*?</a>)?'
    m2 = re.search(ft_pat, c, re.DOTALL)
    if m2:
        c = c[:m2.start()] + STANDARD_FOOTER + c[m2.end():]
        print("  + Footer unified")
    else:
        print("  ! Footer not found")
    
    # 5. Replace text-based logo "R.<span>AGIMOFF</span>" with logo-box img
    c = re.sub(
        r'<a\s+href="[^"]*"\s+class="logo"[^>]*>R<span>\.</span>AGIMOFF</a>',
        '<a href="index.html" class="logo-box"><img src="images/Logo-Ragimoff-Psy.jpg" alt="RAGIMOFF Logo"></a>',
        c
    )

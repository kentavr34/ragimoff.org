# -*- coding: utf-8 -*-
"""Fix haqqinda.html and b2b.html - footer and header standardization"""
import sys, re, os
sys.stdout.reconfigure(encoding='utf-8', errors='replace')

BASE = os.path.dirname(os.path.abspath(__file__))

STANDARD_FOOTER_SIMPLE = """<footer>
  <div class="footer-inner" style="max-width:1300px; margin:0 auto; padding:80px 40px;">
    <div style="display:grid; grid-template-columns:2fr 1fr 1fr 1fr; gap:60px; align-items:start; margin-bottom:60px;">
      <div>
        ...
        <p style="font-size:0.9rem; color:rgba(255,255,255,0.55); line-height:1.7; max-width:280px;">K\u0259nan R\u0259himov &mdash; H\u0259kim-Psixiatr, Psixoterapevt. 23 il klinik t\u0259cr\u00fcb\u0259. Bak\u0131, Az\u0259rbaycan.</p>
...
def fix_file(filename, active):
    ...
    # Add CSS if missing
    if '.mobile-nav {' not in c and '.wa-float {' not in c:
        idx = c.rfind('  </style>')
        if idx == -1:
            idx = c.rfind('</style>')
        if idx != -1:
    ...
    if m2:
        c = c[:m2.start()] + STANDARD_FOOTER_SIMPLE + c[m2.end():]
        print("  + Footer unified")
    else:
        # No footer found - append before </body>
        body_idx = c.rfind('</body>')
        if body_idx != -1:
            c = c[:body_idx] + '\n\n' + STANDARD_FOOTER_SIMPLE + '\n\n' + c[body_idx:]
            print("  + Footer appended before </body>")
...
fix_file('haqqinda.html', 'ana')
fix_file('b2b.html', 'korporativ')
print("Done.")

# -*- coding: utf-8 -*-
"""Standardize header/footer for blog article pages and remaining pages"""
import sys, re, os
...
MOBILE_NAV_CSS = """
    ...
    .mobile-nav .btn-primary {
      margin-top: 16px; text-align: center; background: var(--accent); color: var(--navy);
      padding: 16px; font-weight: 800; border-radius: 2px;
    }
    .mobile-nav-close {
    ...
    .wa-float {
      ...
      box-shadow: 0 8px 25px rgba(37,211,102,0.4); z-index: 1500; transition: transform 0.3s; text-decoration: none;
    }
...
ACTIVE_MAP = {
    'blog-aile.html': 'blog', 'blog-depressiya.html': 'blog',
    'blog-enurez.html': 'blog', 'blog-panik.html': 'blog',
    'blog-psixolog-olmaq.html': 'blog', 'blog-sosial-fobiya.html': 'blog',
    'blog-yeniyetme.html': 'blog',
    'xidmetler.html': 'konsultasiya', 'qanunlar.html': '',
}

FILES = list(ACTIVE_MAP.keys())
...
def process(fn):
    ...
    # Replace old header with text logo
    ...
    if m2:
        c = c[:m2.start()] + STANDARD_FOOTER_FULL + c[m2.end():]
        print('  + Footer unified')
    else:
        # Append before </body>
        body_idx = c.rfind('</body>')
        if body_idx != -1:
            c = c[:body_idx] + '\n\n' + STANDARD_FOOTER_FULL + '\n\n' + c[body_idx:]

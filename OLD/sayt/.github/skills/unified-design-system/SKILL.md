...
description: '**WORKFLOW SKILL** — Single source of truth for all design, typography, spacing, and animation rules. USE FOR: enforcing consistency across all files, validating changes, and generating compliant CSS/HTML. 
RULE #1: Zero Emoji Policy. No emojis in content or code.
RULE #2: Hero grid 1.6fr 1fr. 
RULE #3: Portriat photos must be 380-384px width with premium shadows.'
...
5. **Images & Layout**
   - Hero/About photos: `384px` width (About) / `380px` (Hero), `object-fit: cover`
   - Hero grid: `1.6fr 1fr`
...
- [ ] No emojis anywhere (Global Purge)
- [ ] No inline styles (except critical hero/about tweaks)
- [ ] All padding/margin use `var(--space-X)`
- [ ] h2 margin-bottom is `var(--space-2)` when followed by lead
- [ ] All headings use `text-wrap: balance`
- [ ] All images have `loading="lazy"` (hero: `eager`)
- [ ] Wordmark is `RAGIMOFF. Psixologiya Məktəbi`
- [ ] Finance bars use `#061826` background

...
def main() -> int:
    ...
    # 14. парность инлайновых тегов внутри ячеек и абзацев.
    # Правка меток языка однажды сняла открывающий <span> одной метки и
    # закрывающий другой; общий баланс по файлу при этом сошёлся, потому что
    # ошибки взаимно погасились. Считаем поблочно — так не спрячется.
    unpaired = []
    for lg, d in DIRS.items():
        for fp in sorted(d.glob('*.html')):
            t3 = raw(fp)
            for m in re.finditer(r'<(td|p|li|h[1-4])\b[^>]*>((?:(?!</\1>)[\s\S])*)</\1>', t3):
                body = m.group(2)
                for tag in ('span', 'strong', 'em'):
                    if body.count('<' + tag) != body.count('</' + tag + '>'):
                        unpaired.append(f'{lg}/{fp.stem} <{m.group(1)}> {tag}')
                        break
    ...
    # 17. скобки в абзаце сбалансированы.
    # Азербайджанский мастер держит ноль расхождений на всю книгу — значит
    # это годный эталон, а не придирка. Переводы сломали десять: закрывающая
    # скобка после кода DSM-5-TR терялась, и всё определение оставалось
    # «внутри» скобок. Считается по абзацу: в списках и таблицах скобка
    # законно может переходить из ячейки в ячейку.
    unbalanced = []
    for lg, d in DIRS.items():
        for fp in sorted(d.glob('*.html')):
            m = re.search(r'<main[\s\S]*</main>', raw(fp))
            if not m:
                continue
            for mm in re.finditer(r'<p\b[^>]*>((?:(?!</p>)[\s\S])*)</p>', m.group(0)):
                v = H.unescape(re.sub(r'<[^>]*>', '', mm.group(1)))
                if v.count('(') != v.count(')'):
                    unbalanced.append(f'{lg}/{fp.stem}: {v[:60]}')
    check('скобки_сбалансированы', unbalanced)

    # 18. закрывающая кавычка вплотную к длинному тире:
    # «„лоботомия“— старые методы лечения». Встретилось семь раз, из них
    # дважды в мастере. Справочные страницы пропускаются: там тире стоит
    # внутри ячеек таблицы и склейка мнимая — её создаёт снятие тегов.
    glue = []
    for lg, d in DIRS.items():
        for fp in sorted(d.glob('*.html')):
            if fp.stem in ('abbreviatur', 'terminoloji-luget'):
                continue
            for m in re.finditer('[»”“»]—|—[«„„«]', raw(fp)):

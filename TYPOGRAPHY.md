...
## Классификационное меню превью (Ana səhifə + боковой сайдбар) — КАНОН СТИЛЯ (2026-07-07, требование Кенана)
Единый стиль для ВСЕХ трёх классификаций (XBT-11 / DSM-5-TR / XBT-10). Отличаются только КОДЫ, стиль идентичен.

**Заголовок группы** (глава ICD-11 / класс DSM-5-TR / F-блок МКБ-10), в оглавлении И в боковом меню:
- слева ЗОЛОТОЙ (`var(--gold)`) моноширинный КОД-ДИАПАЗОН, фикс. колонка: ICD-11 `6A00–6A0Z`; DSM `293–301` (min–max по кодам класса, психиатрический блок 285–319); МКБ-10 `F00–F09`.
- справа БЕЛЫЙ (`var(--text)`) жирный заголовок КАПСОМ (az_upper: i→İ, ı→I), фон `var(--bg3)`.

**Подпункт (расстройство):**
- слева ЗОЛОТОЙ моно код своей классификации (ICD-11 код / DSM код / F-код), фикс. колонка (невидимая таблица — коды и названия по одной вертикали).
- название — итоговое АЗЕРБАЙДЖАНСКОЕ КАПСОМ (§0b, одинаковое во всех вкладках).

**Размеры:** вкладки-переключатели .9rem; заголовки групп .95rem жирные; подпункты .9rem. Заголовок НЕ должен быть крупнее/иного стиля, чем соседние — иначе «неопрятно».

**Полнота:** показываем ВСЕ главы/классы/блоки каждой классификации (19 классов DSM-5-TR, 11 F-блоков МКБ-10 вкл. F99). Пустые НЕ выкидываем — помечаем `Bu bölmə tamamlanır` (дополняется).

**Коды:** реальные, выверенные (не «kod təyin olunmamış» для известных болезней!). МКБ-10: Alzheimer F00, сосудистая деменция F01, ФТД F02.0, DLB F02.8, амнестический F04, делирий F05, кататония F06.1; вещества F10-F17 (алкоголь/опиоиды/каннабис/седативные/кокаин/стимуляторы/галлюциногены/никотин); сон F51/G47; секс F52. Источник кодов — `_codes_map.json` (правится вручную). Скрипт `_build_preview.py`.

## Стандартные названия разделов расстройства (КАНОН, 2026-07-07, выверено по az-медисточникам)
Единые для ВСЕХ 103 страниц (не самопал, взято из азерб. медлитературы: UpToDate-az, az.wiki, медучебники):
1. Tərif və nozologiya
2. Tarixçə
3. Epidemiologiya
4. Etiologiya və patogenez
5. **Klinik mənzərə**  ← «клиническая картина» (НЕ «təzahürlər»=проявления; выбор Кенана 2026-07-07)
6. Diaqnoz  (подраздел: Differensial diaqnostika)
...
## Правило имён специалистов/методик/терминов (Кенан 2026-07-07)
- Термин/методика/имя специалиста: СНАЧАЛА азербайджанский → потом (English) в скобках. Читать az-читателю англ. сложнее.
  Пример: `Autizm Diaqnostik Müşahidə Cədvəli (ADOS-2)`, `Kanner (Kanner)`, `Bettelhaym (Bruno Bettelheim)`.
- Медицинские термины разделов — только СТАНДАРТНЫЕ az-медицинские (сверять с az-учебниками/UpToDate-az/az.wiki), не переводить самопалом.
- Перевод: не только 994 (дорого) — можно free API/free router или ресёрч терминов из интернета (WebSearch/WebFetch) с проверкой по источникам (список литературы внизу каждой главы).

## ФИНАЛЬНЫЙ стандарт иностранных слов/методик/аббревиатур/имён (Кенан 2026-07-07, УТОЧНЕНО)
Родной язык страницы (азербайджанский) — ОСНОВНОЙ инструмент. Иностранное — в скобках как международный ориентир.
Азерб. специалисты часто оставляют англ. акронимы из-за нехватки az-литературы — это НЕ эталон, делаем правильно.

- **Методика:** `Азерб. полное название (АЗ-аббревиатура / EN-аббревиатура — English full name)`.
  Пример: `Tətbiqi Davranış Analizi (TDA / ABA — Applied Behavior Analysis)`; `Şəkil Mübadiləsi ilə Kommunikasiya Sistemi (ŞMKS / PECS — Picture Exchange Communication System)`.
  Для методик-акронимов из англ. слов (JASPER, PEERS, TEACCH) АЗ-аббревиатуру не выдумывать — оставить англ. акроним: `Азерб. название (JASPER — English full)`.
- **Аббревиатуры:** АЗ-аббревиатура (в скобках EN-аббревиатура): `XBT-11 (ICD-11)`, `ÜST (WHO)`, `EEQ (EEG)`, `TDA (ABA)`. Первое упоминание на странице — с расшифровкой; повторные — только АЗ.
- **Имена специалистов:** азерб. написание → (оригинал латиницей): `Rocers (Rogers S.)`, `Şopler (Schopler E.)`, `Kögel (Koegel R.L.)`. НЕ оставлять только латиницу.
- Принцип: основной текст на азербайджанском; латиница/международные шаблоны — только в скобках как ориентир (не ошибка, но не основной инструмент).
- ⚠️ Источник точных az-формулировок: ресёрч по достоверным az-источникам; при нехватке — грамотный перевод + созданная АЗ-аббревиатура (Кенан одобрил такой путь; «Erkən» — норм, не туркизм).

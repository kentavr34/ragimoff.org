...
(function () {
  ...
  function initTheme() {
    var KEY = root.getAttribute('data-theme-key') || 'ragimoff-theme';
  ...
  /* ─────────── 5c. ГАЛЕРЕЯ С ПОСЛЕДОВАТЕЛЬНЫМ ПРОЯВЛЕНИЕМ ─────────── */
  /* [data-gal] — секция высотой в N шагов. Прокрутка проявляет кадры по
     одному: каждый ведёт нарратив и появляется своим движением.
     Шаг задаётся data-step (vh на кадр), по умолчанию 36. */
  ...
  function initGallery() {
    ...
    gals.forEach(function (gal) {
      ...
      var N = items.length;
      if (!N) return;
      var step = parseFloat(gal.getAttribute('data-step')) || 36;

      gal.style.setProperty('--xg-n', String(N));
      gal.style.setProperty('--xg-step', step + 'vh');

      gal._items = items;
      gal._num = $('.xg__num b', gal);
      gal._year = $('.xg__year', gal);
      gal._ticks = $$('.xg__tick', gal);
      if (gal._num) gal._num.textContent = '01';
      if (gal._ticks && gal._ticks[0]) gal._ticks[0].classList.add('is-on');
    });
    ...
    function paint() {
      ...
      gals.forEach(function (gal) {
        ...
        gal.style.setProperty('--xg-p', p.toFixed(4));

        var items = gal._items || [];
        var N = items.length;
        if (!N) return;
        var head = p * N;
        var live = Math.min(N - 1, Math.floor(head));

        items.forEach(function (it, i) {
          // кадр раскрывается за 0.55 шага — соседние слегка перекрываются,
          // поэтому проявление не рвётся на дискретные щелчки
          var o = clamp((head - i) / 0.55, 0, 1);
          it.style.setProperty('--o', o.toFixed(4));
          it.classList.toggle('is-live', i === live);
          it.classList.toggle('is-done', i < live);
        });

        if (gal._num) {
          var t = String(live + 1).padStart(2, '0');
          if (gal._num.textContent !== t) gal._num.textContent = t;
        }
        // год активного кадра — тот самый «один такт» с историей
        if (gal._year) {
          var y = items[live].getAttribute('data-year') || '';
          if (y && gal._year.textContent !== y) gal._year.textContent = y;
        }
        if (gal._ticks) {
          gal._ticks.forEach(function (tk, i) { tk.classList.toggle('is-on', i <= live); });
        }
      });
      ticking = false;
    }

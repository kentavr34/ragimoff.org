...
**Разметкой владеют три скрипта, порядок прогона важен:**
`build_headers.py` сам печатает `lang="en"` на `dh-en`; `checkup.py` ищет
`<ol class="ref-list"[^>]*>` с атрибутами; `_build_abbreviatur.py`
перезаписывает `abbreviatur.html` целиком, поэтому **`lang_tags.py` запускать
после него**.
...
**Известный предел:** слово «kültür» в английской глоссе на азербайджанской
странице осталось без метки — строка приходит из `_build_abbreviatur.py`,
который экранирует HTML, и метка вылезла бы текстом.

...
живут английские названия: имя болезни по XBT-11, шкалы, руководства, весь
список литературы. Без метки такой кусок неотличим от родной прозы, и любая
...
Метка — стандартный `lang="en"`, ставит **`lang_tags.py`** (идемпотентен):
* контейнеры по разметке — `<div class="dh-en" lang="en">` и
  `<ol class="ref-list" lang="en">`, 208 на дерево;
* обороты внутри прозы — `<span lang="en">`, ~2170 штук на три дерева.

Английские обороты ищутся по уликам, а не по форме: словарь строится из
самой книги — английский корпус из дерева `en/`, родной из тех отрезков
своего дерева, где есть родная буква. Слово-улика частое в английском и
почти отсутствует в родном (`Guideline`, `Therapy`, `Scale`), поэтому
турецкое `RUHSAL BOZUKLUKLAR` в заглавном регистре английским не считается.

**Разметку теперь владеют два скрипта:** `build_headers.py` сам печатает
`lang="en"` на `dh-en` (иначе пересборка стирала метку), а `checkup.py`
ищет `<ol class="ref-list"[^>]*>` с атрибутами. Меняя эти теги — проверьте оба.

Читателю метка видна тихо: `[lang="en"]` получает пунктир снизу
(`style.css`, все четыре дерева). Список литературы и английское имя в шапке
не подчёркиваются — там английский очевиден и без знака.

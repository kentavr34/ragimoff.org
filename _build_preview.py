...
# режим DEPLOY=1 → выходим в КОРЕНЬ сайта (живая версия), пути корневые (без ../).
# Иначе — в preview/ (пути ../). Исходники читаем из снапшота _srcpages (чтобы деплой их не корраптил).
DEPLOY = os.environ.get("DEPLOY") == "1"
SRC = SITE / "_srcpages"
if not SRC.exists():
    SRC = SITE
OUT = SITE if DEPLOY else (SITE / "preview")
if os.environ.get("OUT_DIR"):          # ручное переопределение (для безопасной проверки деплоя)
...
for seg in re.split(r'(?=<div class="nav-item)', idx):
    ...
    if not ttl:   # записи без кода-диапазона (напр. «Məlumat bölməsi») — берём текст <span>
        ttl = re.search(r'class="nav-link[^"]*"[^>]*>\s*<span>([^<]*)</span>', seg)
...
block = (SRC / "02-6A2-sizofreniya-spektri.html").read_text(encoding="utf-8")
...
EXTRA_CSS = """
<style>
/* приглушённые цвета текста осветлены (тёмно-серый #8B949E/#6E7681 на чёрном плохо читался).
   Стиль темы сохраняем: золото — акценты, светлый текст — основной; только контраст выше.
   Область — контент и боковое меню; шапку САЙТА не трогаем. */
.content-wrap, .sidebar nav{--text2:#C6CDD6; --text3:#AAB2BD}
...
/* методики — каждая отдельным блоком с чётким разделителем и воздухом */
...
table.dh .dh-lbl{display:block;font-family:var(--mono,monospace);font-size:.74rem;letter-spacing:.03em;text-transform:uppercase;color:var(--text3);font-weight:700;line-height:1.25}
table.dh .dh-code{display:block;font-family:var(--mono,monospace);font-weight:700;color:var(--gold);font-size:.74rem;line-height:1.25;margin-top:.1rem}
...
@media(max-width:720px){
  ...
  table.dh h1{font-size:1.12rem;letter-spacing:-.02em;line-height:1.22}
  table.dh .dh-name{font-size:.9rem;line-height:1.28}
  ...
  /* вкладки классификаций — все 3 в одну строку (были переносы XBT-10) */
  .cls-tabs{gap:.35rem;flex-wrap:nowrap}
  .cls-tabs .cls-tab{padding:.45rem .55rem;font-size:.8rem;letter-spacing:0;white-space:nowrap}
...
def page(content, title):
    full = f"{title} | KLİNİK PSİXİATRİYA"
    t = top
    # оболочка берётся из страницы шизофрении → её <title>/og-теги надо заменить на текущую
    # страницу (иначе на всех вкладках/в превью ссылок Telegram/WhatsApp течёт "ŞİZOFRENİYA")
    t = re.sub(r'<title>.*?</title>', lambda m: f'<title>{full}</title>', t, count=1, flags=re.S)
    t = re.sub(r'(<meta name="description" content=")[^"]*(")',
               lambda m: m.group(1) + f'Klinik Psixiatriya — {title}' + m.group(2), t, count=1)
    t = re.sub(r'(<meta property="og:title" content=")[^"]*(")',
               lambda m: m.group(1) + full + m.group(2), t, count=1)
    t = re.sub(r'(<meta property="og:description" content=")[^"]*(")',
               lambda m: m.group(1) + f'Klinik Psixiatriya — {title}' + m.group(2), t, count=1)
    t = re.sub(r'(<meta property="og:url" content=")[^"]*(")',
               lambda m: m.group(1) + 'https://ragimoff.org/klinik-psixiatriya/' + m.group(2), t, count=1)
...
def build_dh(frag, code, fallback_name):
    ...
    # ярлык + код — в столбик (одна узкая ячейка), название — во вторую (широкую)
    def meta(lbl, c):
        return f'<td class="dh-meta"><span class="dh-lbl">{lbl}</span><span class="dh-code">{c}</span></td>'
    rows = [f'<tr class="dh-main">{meta("XBT-11", code)}'
            f'<td class="dh-name"><h1>{az}</h1>{en_html}</td></tr>']
    if i10:
        rows.append(f'<tr>{meta("XBT-10", i10)}<td class="dh-name">{az}</td></tr>')
    if dsm:
        rows.append(f'<tr>{meta("DSM-5-TR", dsm)}<td class="dh-name">{az}</td></tr>')

...
GONE = {
 'az': [
    # 6A70: NESARC-III приписан Kessler; это исследование Hasin и Grant, 2026-08-17
    'Kessler R.C. NESARC-III',
    # 6E61: клиническое противопоказание писалось «qadağa» — термин регуляторного запрета, 2026-08-17
    '(haloperidol) qadağa',
 ...
 'ru': [
    ...
    'встречаются у 30% взрослых мужчин',
    # 6B65 §3: частичное ДРИ сравнивалось с «ПРЛ» — пограничным расстройством
    # личности; три других языка говорят «чем ДРИ», 2026-08-18
    'чаще, чем ПРЛ',
    # 6A21 подгоняла распространённость шизофрении под «вдвое меньше», 2026-08-17
    'вдвое меньше распространённости шизофрении',
    # 7A20: одно сокращение двумя способами в одной карточке — EDS и ЭДС, 2026-08-17
    'при наличии ЭДС',
    # 6E61: то же в русском — «запрещён» вместо «противопоказан», 2026-08-17
    '(галоперидол) запрещён',
    # 6D82 DLB: запрет нейролептиков ослаблялся до «исключены»; при деменции
    # с тельцами Леви реакция гиперчувствительности угрожает жизни, 2026-08-17
    'рисперидон исключены',
    # 6A20 беременность: снят рейтинг «наименее тератогенны», 2026-08-16
 ...
 'en': [
    ...
    # §3: «Spread» — калька с «Yayılma»; по-английски распространённость
    # это Prevalence, книга пишет так в 51 месте, 2026-08-17
    'Spread in children',
    # HA01: сочетание PDE5 с нитратами — жизнеугрожающая гипотензия; запрет
    # был записан вне канона и с существительным в единственном числе, 2026-08-17
    'Never co-administered with nitrate',
    # 6E21: капслок вместо выделения; три остальных языка пишут обычным словом, 2026-08-17
    'ABSOLUTELY NECESSARY',
    # 6E21: причина противопоказания лития потерялась в «(lactation)», 2026-08-17
    'contraindication in breastfeeding (lactation)',
    # 6A20 беременность: снят рейтинг «наименее тератогенны», 2026-08-16
 ...
 'tr': [
    ...
    'Insomnia BİRİNCİ BASAMAK',
    # 6E61: то же в турецком — «yasak» вместо «kontrendike», 2026-08-17
    '(haloperidol) yasak',
    # 6A20 беременность: снят рейтинг «наименее тератогенны», 2026-08-16

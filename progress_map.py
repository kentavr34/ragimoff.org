...
def main() -> int:
    ...
    lines.append('')
    lines.append('Проверяется автоматически, `python checkup.py` — одиннадцать проверок:')
    lines.append('')
    ...
    for name in ('полнота четырёх языков', 'структурная параллельность',
                 ...
                 'известные ложные друзья', 'типографика',
                 'разделитель после </strong>', 'турецкое правило процента',
                 'наличие источников'):

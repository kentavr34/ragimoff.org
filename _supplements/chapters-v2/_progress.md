# Chapters-v2 — autonomous rewrite progress

**Cəlbedici prinsip:** her pozuntu 12-bölməli quruluşa görə tam yazılır (6A20-full.html etalonu).

## Status

| Code | Disorder | Status |
|---|---|---|
...
Hər iterasiyada:
1. İlk `TODO`-nu seçin.
2. `_supplements/chapters-v2/{code}.html` faylını yaradın (12-bölmə, 6A20-full şablonu).
3. Status `TODO` → `DONE`.
4. Hər 4 tamamlanmış pozuntudan sonra git commit + push.
5. Növbəti wakeup ScheduleWakeup ilə (delaySeconds=900).
6. Bütün `TODO`-lar bitdikdə `_done.md` yarat və wakeup planlaşdırma.

## Mənbə hörmət iyerarxiyası

NICE göstərişləri > APA Practice Guidelines > WFSBP > Cochrane > AR Səhiyyə Nazirliyi protokolları.
Hər tövsiyə üçün mənbə göstərilməlidir. Uydurma yox.
